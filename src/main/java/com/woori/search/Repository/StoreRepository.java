...
public interface StoreRepository extends JpaRepository<StoreEntity, Integer> {
    ...
    //StoreEntity u(별칭) : 별칭은 테이블명을 약식표기(보통 알파벳 1글자 지정)
    //Sting keyword => @Param("keyword")==>:keyword
    ...
    // 긴문장을 다중행으로 처리 시 반드시 문자 뒤와 앞에 빈 공백 포함
    ...
    // 모든 항목에서 조회
    Page<StoreEntity> findByStoreChiefIdContainingOrStoreChiefContainingOrStoreChiefIdContainingOrStoreChiefContaining(String keyword1, String keyword2, String keyword3, String keyword4, Pageable pageable);
    @Query(" SELECT u FROM StoreEntity u WHERE "
            +" u.storeName LIKE %:keyword% or u.storeId LIKE %:keyword% "
...
/*
    ...
    3. 지정 필드(Entity의 변수명)로 조회
        ...
        findByStoreName(String name) : name의 값이 storeName필드의 값과 일치하면 조회

    4. 비슷한 값 조회(%값%) => 검색
        findBy필드명Containing(변수) : 변수값이 포함된 데이터를 조회
            예) findByStoreNameContaining(String name) : name의 값이 storeName에 포함된 데이터만 조회

    5. 여러 필드를 사용(AND - 모두 맞으면, OR - ~중 하나라도 맞으면) => 서비스에서 문법이 복잡하다.
        findBy필드명1AND필드명2(변수1, 변수2) : 변수1 값이 필드명1과 같고, 변수2 값이 필드명2와 같으면 조회
            예) findByStoreNameANDStoreId(String name, String id) : name 값이 storeName과 같고, id 값이 storeId와 같으면 조회
                findByStoreNameORStoreId(String name, String id) : name 값이 storeName과 같거나, id 값이 storeId와 같으면 조회
    ...
    8. 페이징 처리
        findBy필드명(변수, Pageable 페이지변수) : 변수값과 같은 데이터를 해당 페이지 부분에서 조회
            예) findByStoreName(String name, Pageable pageable)

    9. 숫자, 날짜 범위 처리
        GreaterThen(>), LessThen(<), GreaterThenEqual(>=), LessThenEqual(<=)
            예) findByAgeGreaterThen(Integer age) : age 값보다 큰 데이터를 조회

    10. 값이 빈 데이터 조회
        findBy필드명IsNull();
            예) findByStoreNameIsNull() : storeName필드에 값이 없는 데이터만 조회
 */
